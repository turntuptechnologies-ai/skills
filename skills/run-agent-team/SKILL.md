...
- **失われるものと補い方**: tmux での可視化とメンバー間の直接通信は使えない。lead が各ロールの報告を要約してユーザーに逐次共有し、判断の記録は Agent Teams 時と同じく Issue コメント（手順 6）で残す。
...
- **ロールの完了報告は根拠を確かめてから受け取る。** 「テストが通った」はテスト出力や PR の checks、「実装した」は差分、「ドキュメントを直した」は該当ファイルで裏を取ってから次ロールへ進める。理由: 報告は成果の要約であり、実物とずれていても次ロールはそれを前提に進んでしまう。
