...
- **目標と制約を書き、手順書にしない** — モデルが既に知っている操作を細かく指示しない（過剰な指示は品質を下げる）。正確なコマンドを載せるのは間違えると壊れる操作に限り、制約には理由を添え、強調語は譲れない 1〜2 点に限る。移行相対表現（「旧 ◯◯」）や経緯の語りは書かず、現行ルールだけを書く。「よく考えて」等の思考を促す定型句や推論過程の書き出し要求も書かない（思考の量はモデルが決める）。
