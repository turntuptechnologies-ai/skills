...
- 「よく考えて」「ステップバイステップで」等の思考を促す定型句や、推論過程を返答に書き出させる指示は書かない（モデルは自分で思考の量を決める。推論の再現要求は安全フィルタに掛かることがある）。判断の理由・根拠を成果物に残させるのはよい。
- ユーザー確認で止まるのは、ユーザーにしか決められない判断と、破壊的・外部公開の操作の前に限る。それ以外は止まらずに進め、状況報告は次の行動と同じメッセージで行わせる。
