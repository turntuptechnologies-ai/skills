...
- `skill-lint` にチェック項目 11「過剰指示なし」を追加。手順が目標・制約の粒度か、強調語が譲れない制約に限られているか、移行相対表現（「旧 ◯◯」）が無いかを判定する。間違えると壊れる操作のコマンド列挙・テンプレ必須の太字・自明な制約は対象外 (#91)
- `templates/SKILL.md` の執筆コツと README 運用ルールに同じ執筆基準（モデルが知っている操作を手順書にしない・制約に理由を添える・現行ルールだけを書く）を追記 (#91)

### Changed

- `create-pr` — VCS 操作の手順書を目標＋制約の記述に書き換え（ブランチ命名・Conventional Commits・`Closes #N`・URL 報告の制約は不変） (#91)
- `debug-root-cause` — 切り分け手段の列挙を 1 行に圧縮（再現→原因→red テスト→修正→水平展開の順序は不変） (#91)
- `run-agent-team` — 「旧 designer」の表記を削除。設計レビュー通過後の developer / tester の並行作業と、lead が待ち時間に止まらない運用を追記 (#91)
